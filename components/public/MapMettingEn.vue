...
<script>
import AMapJS, { AMapLoader, AMapUILoader, LocaLoader } from 'amap-js'
...
export default {
    ...
    head() {
        return {
            script: [{ src: `https://webapi.amap.com/maps?v=1.4.15&key=${this.$config.amapKey}` }],
        }
    },
    watch: {
        mapData: {
    ...
    methods: {
        async getData(data) {
            ...
            try {
                ...
                for (let i = 0; i < data.addressList.length; i++) {
                    ...
                    this.markersMany(AMap, this.Maps, location, item)
                }
        ...
        initMap() {
            ...
            return new Promise((resolve, reject) => {
                ...
                AMap.plugin(['AMap.ToolBar', 'AMap.Scale', 'AMap.MapType', 'AMap.Geolocation', 'AMap.Driving', 'AMap.Geocoder'], () => {
        ...
        markersMany(AMap, map, provinces, data) {
            ...
            markers.setLabel({
                ...
                content:
                    `<a href='https://www.amap.com/search?query=${data.address}' target='_blank' class='flex'>` +
